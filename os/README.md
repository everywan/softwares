...
- 公司: 电源 + 主(从)显示器 + cherry g80-3000 + 任意无线鼠标 + typec 扩展坞(方便nuc移动)
    ...
    - 键盘: 试过 hhkb 的, 太不好用了. 还是cherry吧, 在系统里改下键位映射, 键帽换下即可. 也可以试下 flico 的 87/60 键盘.
- 家里: 假设家里配备了电视, 专门用于看电影等.
    - 方案一: 电源 + 随便无线键鼠套装 + 电视
        - 待检测: 此方案是否可行
        - 电视是不可缺少的, 用于看电影, 3A游戏(主机) 等.
    - 方案二: 电源 + 单显示器 + ikbc c87 + 雷蛇鼠标 + type c 扩展坞
        - 家里有电视, 所以对显示器需求不高, 随便来个就行(最好是tn屏的, 频率比ips的高, 虽然效果差些)
        - ikbc+雷蛇 也可以替换为 无线键鼠套装, 手感可能稍差些, 根据需求自行决定吧.


待验证
1. 在公司 arch 是否需要4k显示器: 显示效果, 屏幕大小等.
2. 在家 电视可否取代显示器: fps游戏帧率, 效果, 以及配合无线套装能否用于干活(nuc时).
3. 无线键鼠套装 / 机械键盘+雷蛇 差别有多大.
4. 是否需要升级cherry键盘
    - 无线双模需求
    - 104键=>87键, 节省不必要的空间.(67键位就算了, 布局太紧凑了)
    - 手感差别应该不算很大, 毕竟轴都是一样的, 就是键位差别有点大.